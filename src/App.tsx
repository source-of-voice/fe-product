...
function isAllowedAudioFile(file: File) {
  ...
  return (
    ALLOWED_AUDIO_TYPES.includes(normalizedType) ||
    ...
    return (
      <main className="auth-page">
        ...
        <section className="auth-panel glass-card">
          ...
          {notice && (
            <p className={`notice notice--${notice.kind}`}>{notice.message}</p>
          )}
        </section>
      </main>
    );
      ...
      <div className="app-container">
        <div className="app-workspace">
          ...
          <section className="main-workspace">
            ...
            {notice && (
              <p className={`notice notice--${notice.kind}`}>
                {notice.message}
              </p>
            )}
            ...
            {view === "tasks" && (
              <TasksView
                language={languageCodeFromI18n(i18n.language)}
                setNotice={setNotice}
              />
            )}
...
function TasksView({
  ...
  const [tasks, setTasks] =
    useState<SliceResponse<UserAudioTextListItemResponse>>(emptySlice());
  const [submissions, setSubmissions] =
    useState<SliceResponse<AudioSubmissionResponse>>(emptySlice());
  ...
  const [selectedTask, setSelectedTask] =
    useState<UserAudioTextDetailsResponse | null>(null);
  ...
  const visibleTasks = useMemo(() => {
    ...
    const filtered = normalized
      ? tasks.content.filter((task) =>
          task.sourceTitle?.toLowerCase().includes(normalized),
        )
      : [...tasks.content];
    ...
    return filtered.sort((a, b) => {
      switch (sort) {
        ...
        case "difficultyAsc":
          return (
            Number(a.difficultyScore ?? 0) - Number(b.difficultyScore ?? 0)
          );
        case "difficultyDesc":
          return (
            Number(b.difficultyScore ?? 0) - Number(a.difficultyScore ?? 0)
          );
        case "wordsAsc":
      ...
      <div className="tasks-layout">
        <section className="glass-card panel-section">
            ...
            {visibleTasks.map((task) => (
              ...
              >
                <div>
                  ...
                  <p>
                    {task.wordCount} {t("words")} ·{" "}
                    {task.estimatedReadingSeconds}s · {t("difficulty")}{" "}
          ...
          <Pagination
            page={taskPage}
            last={tasks.last}
            onPrevious={() => loadTasks(Math.max(0, taskPage - 1))}
            onNext={() => loadTasks(taskPage + 1)}
          />
        </section>
        ...
        <section className="glass-card panel-section">
          ...
          <Pagination
            ...
            onPrevious={() => loadSubmissions(Math.max(0, submissionPage - 1))}
            onNext={() => loadSubmissions(submissionPage + 1)}
          />
        </section>
      </div>
...
function TaskModal({
  ...
  task: UserAudioTextDetailsResponse;
  onClose: () => void;
  onSubmitted: () => void;
  setNotice: (notice: Notice) => void;
}) {
  ...
  const startRecording = async () => {
    try {
      ...
      recorder.onstop = () => {
        const normalizedType = normalizeAudioType(
          recorder.mimeType || recordingMimeType,
        );
        const extension = extensionForAudioType(normalizedType);
        const blob = new Blob(chunksRef.current, { type: normalizedType });
        const file = new File(
          [blob],
          `source-of-voice-recording.${extension}`,
          { type: normalizedType },
        );
  ...
  return (
    <div className="modal-overlay" role="presentation">
      <section
        className="glass-modal task-modal"
        role="dialog"
        aria-modal="true"
        ...
        <div className="task-modal-grid">
          <article className="glass-modal-section reading-panel">
            <div className="metric-row">
              ...
              <span>
                {t("difficulty")} {task.difficultyScore}
              </span>
            </div>
            <h3>{t("textToRead")}</h3>
          ...
          <article className="glass-modal-section record-panel">
            ...
            <label className="file-dropzone">
              <input
                ...
                onChange={(event) => {
                  ...
                  if (file && !isAllowedAudioFile(file)) {
                    setNotice({
                      kind: "error",
                      message: t("unsupportedAudioFormat"),
                    });
                    event.target.value = "";
                    return;
                  }
              ...
              <span>
                {selectedFile?.name ??
                  recordedFile?.name ??
                  t("uploadExistingFile")}
              </span>
              {(selectedFile ?? recordedFile) && (
                <small>{fileSize((selectedFile ?? recordedFile)!.size)}</small>
              )}
            </label>
...
function ReviewerView({ setNotice }: { setNotice: (notice: Notice) => void }) {
  ...
  const [details, setDetails] =
    useState<ReviewerAudioSubmissionDetailsResponse | null>(null);
  ...
  const load = async () => {
    ...
    try {
      const [availableData, assignedData] = await Promise.all([
        reviewerApi.available(),
        reviewerApi.assigned(),
      ]);
    ...
    <div className="reviewer-grid">
      <section className="glass-card panel-section">
        ...
        <ReviewList
          ...
          onClaim={(id) => runAction(() => reviewerApi.claim(id))}
        />
      </section>
      ...
      {details && (
        <section className="glass-card panel-section reviewer-details">
          <div className="section-heading roomy">
            ...
            <StatusBadge tone={toneForStatus(details.status)}>
              {details.status}
            </StatusBadge>
          </div>
          <div className="detail-grid">
            ...
            <aside className="review-actions glass-card-soft">
              <InfoPill
                icon={<Coins size={18} />}
                label={`${t("payout")}: ${formatMoney(details.payoutAmount)}`}
              />
              <InfoPill
                icon={<Gauge size={18} />}
                label={`${t("score")}: ${details.correctnessScore ?? "—"}`}
              />
              <button
                className="secondary-button full"
                onClick={() => playAudio(details.id)}
              >
                <Headphones size={18} />
              ...
              <div className="button-row stacked">
                <button
                  ...
                  onClick={() =>
                    runAction(() => reviewerApi.approve(details.id))
                  }
                >
                  <BadgeCheck size={18} />
                  <span>{t("approve")}</span>
                ...
                <button
                  ...
                  onClick={() =>
                    runAction(() => reviewerApi.reject(details.id))
                  }
                >
                  <X size={18} />
                  <span>{t("reject")}</span>
...
function WalletView() {
  ...
  const [transactions, setTransactions] =
    useState<SliceResponse<WalletTransactionResponse>>(emptySlice());
  ...
  return (
    <div className="wallet-grid">
      <StatCard
        icon={<WalletCards size={22} />}
        label={t("balance")}
        value={loading && !wallet ? t("loading") : formatMoney(wallet?.balance)}
      />
      <section className="glass-card panel-section wide-panel">
        ...
        <Pagination
          ...
          onPrevious={() => loadTransactions(Math.max(0, transactionPage - 1))}
          onNext={() => loadTransactions(transactionPage + 1)}
        />
      </section>
    </div>
  );
...
function AdminView({ setNotice }: { setNotice: (notice: Notice) => void }) {
  const { t } = useTranslation();
  const [generatePayload, setGeneratePayload] =
    useState<GenerateWikipediaTextsRequest>({
      languageCode: "en",
      requestedCount: 5,
      minWords: 25,
      maxWords: 100,
  ...
  const [batches, setBatches] =
    useState<SliceResponse<AdminAudioTextBatchListItemResponse>>(emptySlice());
  const [texts, setTexts] =
    useState<SliceResponse<AdminAudioTextListItemResponse>>(emptySlice());
    ...
    <div className="admin-layout">
      <section className="glass-card panel-section wide-panel">
        ...
        <div className="admin-form-grid">
          <Field label={t("languageCode")}>
            <input
              value={generatePayload.languageCode}
              onChange={(event) =>
                setGeneratePayload({
                  ...generatePayload,
          ...
          <Field label={t("requestedCount")}>
            <input
              type="number"
              value={generatePayload.requestedCount}
              onChange={(event) =>
                setGeneratePayload({
                  ...generatePayload,
          ...
          <Field label={t("minWords")}>
            <input
              type="number"
              value={generatePayload.minWords}
              onChange={(event) =>
                setGeneratePayload({
                  ...generatePayload,
          ...
          <Field label={t("maxWords")}>
            <input
              type="number"
              value={generatePayload.maxWords}
              onChange={(event) =>
                setGeneratePayload({
                  ...generatePayload,
          ...
          <Field label={t("baseRatePerWord")}>
            <input
              value={String(generatePayload.baseRatePerWord)}
              onChange={(event) =>
                setGeneratePayload({
                  ...generatePayload,
                  baseRatePerWord: event.target.value,
                })
              }
            />
          </Field>
          <Field label={t("wikipediaFetchLimit")}>
            <input
              type="number"
              value={generatePayload.wikipediaFetchLimit}
              onChange={(event) =>
                setGeneratePayload({
                  ...generatePayload,
                  wikipediaFetchLimit: Number(event.target.value),
                })
              }
            />
          </Field>
        ...
        <div className="toggle-row">
          <label>
            <input
              type="checkbox"
              checked={generatePayload.activateImmediately}
              onChange={(event) =>
                setGeneratePayload({
                  ...generatePayload,
                  activateImmediately: event.target.checked,
                })
              }
            />{" "}
          ...
          <label>
            <input
              type="checkbox"
              checked={generatePayload.introOnly}
              onChange={(event) =>
                setGeneratePayload({
                  ...generatePayload,
                  introOnly: event.target.checked,
                })
              }
            />{" "}
            {t("introOnly")}
        ...
        <button
          className="primary-button"
          onClick={() => run(() => adminTextApi.generate(generatePayload))}
        >
      ...
      <section className="glass-card panel-section wide-panel">
        <div className="section-heading roomy">
          ...
          <select
            value={statusFilter}
            onChange={(event) =>
              setStatusFilter(event.target.value as AudioTextStatus | "")
            }
          >
          ...
          {texts.content.map((text) => (
            <article key={text.id} className="compact-row">
              <div>
                ...
                <p>
                  {text.languageCode} · {text.wordCount} {t("words")}
                </p>
                <StatusBadge tone={toneForStatus(text.status)}>
                  {text.status}
                </StatusBadge>
              </div>
              <div className="button-row wrap">
                <button
                  className="secondary-button small-button"
                  onClick={() => run(() => adminTextApi.activate(text.id))}
                >
                  {t("activate")}
                </button>
                <button
                  className="secondary-button small-button"
                  onClick={() => run(() => adminTextApi.disable(text.id))}
                >
                  {t("disable")}
                </button>
                <button
                  className="secondary-button small-button"
                  onClick={() => run(() => adminTextApi.archive(text.id))}
                >
                  {t("archive")}
                </button>
              </div>
        ...
        <Pagination
          page={textPage}
          last={texts.last}
          onPrevious={() => loadTexts(Math.max(0, textPage - 1))}
          onNext={() => loadTexts(textPage + 1)}
        />
      </section>
      ...
      <section className="glass-card panel-section wide-panel">
          ...
          {batches.content.map((batch) => (
            <article key={batch.id} className="compact-row">
              <div>
                ...
                <p>
                  {batch.languageCode} · {batch.savedCount}/
                  {batch.requestedCount} · {t("skippedCount")}:{" "}
                  {batch.skippedCount}
                </p>
              </div>
        ...
        <Pagination
          page={batchPage}
          last={batches.last}
          onPrevious={() => loadBatches(Math.max(0, batchPage - 1))}
          onNext={() => loadBatches(batchPage + 1)}
        />
      </section>
    </div>
  );
    ...
    <div className="settings-grid">
      <section className="glass-card panel-section settings-card">
        ...
        <div className="form-stack">
          ...
          <button
            className="primary-button"
            onClick={() => run(() => authApi.changeEmail(email))}
          >
            {t("save")}
          </button>
        </div>
      ...
      <section className="glass-card panel-section settings-card">
        ...
        <div className="form-stack">
          ...
          <button
            className="primary-button"
            onClick={() => run(() => authApi.changeUsername(username))}
          >
            {t("save")}
          </button>
        </div>
