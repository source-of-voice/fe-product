...
export const audioApi = {
  ...
  mySubmissions(page = 0, size = 20) {
    return apiRequest<SliceResponse<AudioSubmissionResponse>>(`/sofv/audio/my${query({ page, size })}`);
  }
};

export const reviewerApi = {
  available(page = 0, size = 20) {
    return apiRequest<SliceResponse<AudioSubmissionResponse>>(`/sofv/reviewer/audio/available${query({ page, size })}`);
  },
  assigned(page = 0, size = 20) {
    return apiRequest<SliceResponse<AudioSubmissionResponse>>(`/sofv/reviewer/audio/my${query({ page, size })}`);
  },
  details(id: number) {
    return apiRequest<ReviewerAudioSubmissionDetailsResponse>(`/sofv/reviewer/audio/${id}`);
  },
  claim(id: number) {
    return apiRequest<AudioSubmissionResponse>(`/sofv/reviewer/audio/${id}/claim`, { method: 'PATCH' });
  },
  approve(id: number) {
    return apiRequest<AudioSubmissionResponse>(`/sofv/reviewer/audio/${id}/approve`, { method: 'PATCH' });
  },
  reject(id: number) {
    return apiRequest<AudioSubmissionResponse>(`/sofv/reviewer/audio/${id}/reject`, { method: 'PATCH' });
  },
  file(id: number) {
...
export const adminTextApi = {
  generate(payload: GenerateWikipediaTextsRequest) {
    return apiRequest<GenerateWikipediaTextsResponse>('/sofv/admin/texts/generate', {
      method: 'POST',
      body: JSON.stringify(payload)
    });
  },
  ...
  batchDetails(id: number) {
    return apiRequest<AdminAudioTextBatchDetailsResponse>(`/sofv/admin/textBatches/${id}`);
  },
  ...
  activate(id: number) {
    return apiRequest<AudioTextStatusResponse>(`/sofv/admin/texts/${id}/activate`, { method: 'PATCH' });
  },
  disable(id: number) {
    return apiRequest<AudioTextStatusResponse>(`/sofv/admin/texts/${id}/disable`, { method: 'PATCH' });
  },
  archive(id: number) {
    return apiRequest<AudioTextStatusResponse>(`/sofv/admin/texts/${id}/archive`, { method: 'PATCH' });
  }
};
...
export const walletApi = {
  ...
  transactions(page = 0, size = 20) {
    return apiRequest<SliceResponse<WalletTransactionResponse>>(`/payments/wallet/me/transactions${query({ page, size })}`);
  }
};
